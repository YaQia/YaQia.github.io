...
- 异步变量的更新有延迟，不能立刻渲染，导致与 signal 的不一致。现在的 computational 变量（即会影响渲染的变量）不光有 signal 还有 `Promise` 等 async 变量，因此假如 signal 变量修改即立刻渲染，而 `Promise` 则必须在依赖的 signal 修改后再发出请求等待结果，那么从 UI 上来看，所有 signal 变量都迅速被更新了而 `Promise` 则还没有。这是一种 UI 渲染的不一致。
